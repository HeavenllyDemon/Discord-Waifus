...
| **P3 — Orchestration cutover** (P3a ✅ done 2026-06-13) | Rewrite `ModelPipeline` on gateway client; move prompts/tools; delete `pipelines.ts` + `catalog.ts`. **P3a done** (`docs/superpowers/plans/2026-06-12-gateway-p3a-pipeline-on-gateway.md` + execution record): all six methods live on the gateway behind `createGatewayModelPipeline`, side-by-side, 631 tests green, fake-transport goldens across all four wires; found+fixed a latent Gemini flat-dream-op bug pre-cutover. **P3b pending**: cutover wiring + legacy-id map, /api/models rewire, deletions, live smoke | P3a met for the fake-transport half; live smoke lands with P3b |
