...
| **P1 — Gateway core** | New repo: registry, validate, codecs, transport, client, server, sync, tests. Split into three plans: **P1a registry+validation ✅ done 2026-06-10** (repo live at `../waifucave-gateway`, 32 tests; see `docs/superpowers/plans/2026-06-10-gateway-p1a-registry-validation.md` execution record); **P1b codecs/transport/client ✅** and **P1c server+sync ✅ done 2026-06-12** (repo at 52 commits/224 tests at P2 time; last P1c carryovers — malformed-element 400s, sync-shape live verification — closed in P6) | `npm test` green; `gateway serve` answers all 5 endpoints; golden fixtures cover every quirk in Table A rows 1–35 |
| **P2 — Side-by-side** ✅ done 2026-06-12 | Add `file:../gateway` dep; mount `/api/llm/*`; `/api/models` proxies registry. `pipelines.ts` still serves traffic. Executed per `docs/superpowers/plans/2026-06-11-gateway-p2-side-by-side.md` (see its execution record): 5 commits `fa87860`→`7ba5444`, app-repo only, gateway untouched at `27b0def` | Met: both lists pinned by tests (`/api/models` 23 legacy + 100 gateway; `/api/providers` 6 + 14); zero diffs in `src/providers`/`src/orchestration`/schemas; 382 tests green |
| **P3 — Orchestration cutover** ✅ done 2026-07-01 | **P3a done 2026-06-13** (`docs/superpowers/plans/2026-06-12-gateway-p3a-pipeline-on-gateway.md`): all six methods on the gateway behind `createGatewayModelPipeline`, side-by-side, fake-transport goldens across all four wires; found+fixed a latent Gemini flat-dream-op bug pre-cutover. **P3b done 2026-07-01** (`docs/superpowers/plans/2026-06-13-gateway-p3b-cutover.md` + execution record): commits `8ac40d2`→`7879f13` — resolver with §7.3 legacy-id map, runtime + persona digest cutover, `/api/models` legacy fields synthesized from the registry (30 models), `pipelines.ts`/`catalog.ts`/`pipelines.test.ts` deleted (−5,425 lines), eval harness swapped, live smoke on anthropic/openai + deepseek wire-shape (real system turns confirmed post data-fix). Live smoke + final review caught three legacy-leniency regressions, all fixed with parity tests: orchestrator decision Raw normalization (`7efa71d`), tool-only waifu replies (empty content + tool calls), per-op dream-op tolerance (`7879f13`). Deferred minors for P4/P5: gate wire `tools` on `supportsTools` (moot today — all 100 registry models support tools); restore digest credential-preflight 409; give `GatewayPipelineError` a `details` field carrying raw model text (`summarizeProviderPipelineDetails` currently dead); forced-call text-content fallback dropped (forced toolChoice verified live). | Met: 541 tests green; suite/typecheck/build clean; gateway serves 100% of chat traffic; legacy ids remap via `LEGACY_MODEL_MAP` |
| **P4 — Storage + domain** ✅ done 2026-07-02 | Executed per `docs/superpowers/plans/2026-07-01-gateway-p4-storage-domain.md` (+ execution record), commits `056d783`→`3ed9bfa`: `params` record replaces `reasoning`/`generation` on Agent/Waifu configs (shared `src/shared/paramsCompat.ts`); server-side compat layer keeps the untouched SPA working until P5 (GET synthesizes legacy views; PUT accepts legacy bodies — legacy fields authoritative over echoed params after a review-caught SPA-echo bug); schema v2 + boot migration (`migrate-configs-to-gateway-params`, `stamp-schema-version-2`) with §7.3 remap; `ProviderIdSchema` → string with registry-validated writes (openrouter keys storable); `gateway.validate()` → 400 `unsupported_parameter` naming the violated rule; doctor warns on unresolved models + unstamped files and tolerates unmigrated v1 roots; P3b minors landed (digest credential-preflight 409, `GatewayPipelineError.details`, `ProviderPipelineError` deleted). **Sanctioned deviation from §7.3:** unmappable ids left in place + doctor warning (no silent substitute) — consequence: such configs 400 on SPA edits until repointed (live data: `waifu:lumi` / `xai/grok-4-1-fast-non-reasoning`). Deferred to P5/P6: normalize-legacy-ids-on-write policy; digest-compat test; non-representable-key wipe on SPA legacy-only waifu saves. | Met: 588 tests green; live-copy smoke — real v1 root migrated exactly, compat GET/PUT, validation 400s, openrouter persisted, doctor caught a real stale model id |
| **P5 — Frontend** ✅ done 2026-07-02 | Executed per `docs/superpowers/plans/2026-07-02-gateway-p5-frontend.md` (+ execution record), commits `436808e`→`3302cac`: SPA driven by the gateway capability surface — `src/frontend/api/llm.ts` (type-only gateway imports, memoized with rejection-reset) + pure `modelParams/logic.ts`; descriptor-driven `ModelParamsForm` (slider/select/toggle/tag-list, unverified pills, debounced `POST /v1/validate` with inline violations + Save gating; `responseFormat` excluded by key); two-level model→route picker (direct-preferred, unknown stored ids kept selected as "(unavailable)"); all five §7.6 views cut over with explicit native-`params` bodies; ProvidersView lists all 14 registry providers; P4 compat layer RETIRED (`feat!:` `3302cac`). **§7.6 mechanism deviation (sanctioned):** "constraint rules live" = reactive endpoint validation (inline errors + forced/clamped/dropped warnings + Save gating), not preemptive control disabling; `effectiveParams` unused. Review loops caught: committed NUL byte making llm.ts git-binary; memo poisoning; boolean params unclearable; **zod `.partial()` manufacturing defaults into PATCH merges (Critical — fixed root-cause across Agent/Waifu/Server body schemas; this same bug pre-P4 had silently damaged the live stage-manager config during the P3b smoke — repaired from in-session values)**. P6 work items filed: explicit-`null` unset contract for providerId/modelId; `DiscordBotsBodySchema` latent default-gotcha; legacy `/api/models`+`/api/providers` + `api.models()` removal; TagListField blur/save ordering. | Met: 644 tests green; bundle free of gateway value-code; headless smoke on live-root copy (native shapes, violations, lumi 404/unknown_model, PATCH preservation); visual browser pass deferred to the user |
| **P6 — Harden** ✅ done 2026-07-02 | Executed per `docs/superpowers/plans/2026-07-02-gateway-p6-harden-publish.md` (+ execution record). Audit found 0.1.0 already published + pinned (release 1.5.156) but DRIFTED (npm lacked the DeepSeek data fix; local dev ran a stale `file:`-era symlink). Gateway `4740540`→`fa6c4c1`: malformed-element 400s at the HTTP boundary (P1c carryover closed — many cases were silently mis-encoded 200s, worse than the hypothesized 500s; role-scoped block validation), LICENSE, CI (test + live drift check + publish-freshness guard — the guard class that would have caught the 0.1.0 drift; the drift job caught REAL upstream drift on its first run: 2 stale openrouter routes removed, 14 pricing cells synced to live values), **0.1.1 published (user-confirmed)**. App `88102b1`→`bdd3378`: `/api/models` deleted; `/api/providers` → credentials-status over all 14 providers (redacted hints, docsUrl map) + `gatewayProviders`; write contracts — explicit-`null` unset (both merges prune undefined post-spread), **normalize-legacy-ids-on-write (supersedes P4's store-literal deviation)**, personaDigest omitted on create, DiscordBots `enabled` required in bodies; TagListField blur/save race PROVEN NON-BUG via react-dom 19.2.6 source trace; pin → **0.1.1 via real npm install (symlink dead)**; live `gateway sync` with real keys clean (P1c sync-shape carryover closed). Still open post-migration: anthropic responseFormat codec gap (deferred, no UI consumer — §11); lumi unresolvable model (user decision); gemini-2.0-flash registry removal (user decision); xai /models 403 (account-side). | Met: app installs from npm (`npm ci`, no symlink, no-file-deps passes, 653 tests green against the published package); `gateway sync` clean; gateway CI green |
...
> **Deferred (P6, 2026-07-02): anthropic-messages `responseFormat`.** The registry truthfully marks anthropic models `json_schema`-capable, but the anthropic-messages codec deliberately throws a clean 400 ("use tools for structured output") instead of implementing `output_config.format`. No UI consumer exists (P5 excludes the control by key); the app's structured outputs are tools-based. Implement in the codec (single-file change + live probes) whenever a consumer appears.
