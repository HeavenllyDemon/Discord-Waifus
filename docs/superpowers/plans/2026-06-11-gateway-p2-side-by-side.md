...
Smoke outcome: full pipeline verified live — stored key → sync lookup → plugin → codec wire body → real provider → error taxonomy → HTTP envelope. A 200 completion wasn't reachable from this machine for account reasons only: the local DeepSeek key is invalid (provider 401 "Authentication Fails … ****83d4") and the xAI team is out of credits (provider 403 mapped to the auth taxonomy). Registry GETs, `/api/models` (23+100), `/api/providers` (6+14), and live credentialConfigured flips all verified against the running server.
