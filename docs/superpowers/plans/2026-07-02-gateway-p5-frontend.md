...
- Render `buildParamControls(doc)` grouped under two `section-title`s ("Sampling", "Reasoning") + "Advanced" for `other`. Control per descriptor.type: `number`/`int` with min+max → `RangeField` (range input + synced number input, step from descriptor, int coerces); without both bounds → plain `.input` number; `boolean` → `Toggle`; `enum` → `.select` over `descriptor.values`; `string[]` → `TagListField` (chips + text entry, enforce `maxItems`); `string` → `.input`; `map` → skip (no UI). ALSO skip by KEY: `responseFormat` (AMENDED after T1 review: moonshot's `responseFormat` descriptor is `type:"enum"`, so type-based skipping alone would render the forbidden structured-output control — the Global Constraint exclusion must be keyed on the param name; add a `buildParamControls` exclusion + test in logic.ts). Unset params render empty with `descriptor.default` as placeholder — an untouched control stores NO key (absent ≠ default).
